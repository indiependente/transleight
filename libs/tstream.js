...
var tstream = function(lang){
				...
				var options = {
					...
					var res_stream = request(options, function(err, res, body){
						...
						if (!err && res.statusCode == 200) {
							...
							if (encoding && (encoding.indexOf('gzip') >= 0 || encoding.indexOf('deflate') >= 0)) {
								const buffer = Buffer.from(body);
								zlib.unzip(buffer, function (err, unzippedbuffer) {
									if (err)
										throw err;
									translator.emit('data', unzippedbuffer);
								});
							}
							else {
								console.log('plain');
	...
	return combine(chunknlines(), encoder, translator, decoder);
